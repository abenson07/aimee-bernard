...
Its own Vercel project, separate from the `web/` Astro site's project, with
its own URL (Root Directory: `dashboard`). Set the env vars above on it and
deploy — no shared domain, no proxying, nothing else app-specific to
configure.

`/dashboard` redirects to `/` so older links keep working.
